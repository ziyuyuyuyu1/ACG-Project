...
def adjust_text_embeddings(embeddings, azimuth, opt=None):
    text_z_list = []
    weights_list = []
    K = 0
    for b in range(azimuth.shape[0]):
        if opt is not None:
            ...
            text_z_ = get_pos_neg_text_embeddings(embeddings, azimuth[b], opt) 
...
def get_pos_neg_text_embeddings(embeddings, azimuth_val, opt=None):
