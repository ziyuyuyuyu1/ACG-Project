...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16=False, vram_O=False, sd_version='2.1', hf_key=None, t_range=[0.02, 0.98]):
        ...
        if hf_key is not None:
            ...
            model_key = "/share1/jialuo/models/stable-diffusion-2-1-base"
        else:
    ...
    @torch.no_grad()
    def get_text_all_embeddings(self, prompt, prompt_neg, azimuth):
        embeddings = {}
        embeddings['default'] = self.get_text_embeds([prompt])
        embeddings['uncond'] = self.get_text_embeds([prompt_neg])
        print(embeddings['uncond'].shape)
        azimuth = azimuth.to(self.device)
        
        text_z = [embeddings['uncond']] * azimuth.shape[0]

        for d in ['front', 'side', 'back']:
            embeddings[d] = self.get_text_embeds([f"A 3D model of {prompt}, white background, {d} view"])
            # print(f"embeddings[{d}].device: {embeddings[d].device}")
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=10, as_latent=False, grad_scale=1,
