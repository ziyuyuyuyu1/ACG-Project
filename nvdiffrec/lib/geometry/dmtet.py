...
from .dmtet_fixedtopo import DMTetGeometryFixedTopo
import json
import kaolin
...
class DMTetGeometry(torch.nn.Module):
    def __init__(self, grid_res, scale, FLAGS, root='./', grid_to_tet=None, deform_scale=1.0, load_tet=False, path=None, **kwargs):
        ...
        if self.load_tet:
            ...
            sdf = tet['sdf'].to(FLAGS.device)
            deform = tet['deform'].to(FLAGS.device)
            self.deform = torch.nn.Parameter(deform.clone(), requires_grad=False)
            self.sdf    = torch.nn.Parameter(sdf.clone().detach(), requires_grad=False)
        else:    
            sdf = torch.rand_like(self.verts[:,0]).clamp(-1.0, 1.0) - 0.1
            deform = torch.zeros_like(self.verts)
            self.deform = torch.nn.Parameter(deform.clone(), requires_grad=True)
    ...
    def getMesh(self, material, noise=0.0, ema=False):
        ...
        print(f'vertices: {verts.shape}, uvs: {uvs.shape}, uv_idx: {uv_idx.shape}')
    ...
    def gen_img(self, glctx, lgt, opt_material, xfm_lgt=None, val_save_name=None, batch=None):
        ...
        imesh, buffers = self.render_with_mesh(glctx, final_target, lgt, opt_material, noise=0.0, xfm_lgt=xfm_lgt)
        # print(opt_material['kd'].data)
        img_list = []
        for i in range(batch):
            img = render_utils.rgb_to_srgb(buffers['shaded'][...,0:3])[i]
            # img = buffers['shaded'][...,0:3][i]
    ...
    def tick_text(self, glctx, lgt, opt_material, prompt=None, sd=None, FLAGS=None, iteration=None):
        ...
        imesh = self.getMesh(opt_material, noise=0.0, ema=False)
        reg_loss = torch.tensor(0.0).cuda()
