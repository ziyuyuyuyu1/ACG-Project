...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps, grid_mask=None, return_traj=False, classifier_fn=None):
...
class AncestralSamplingPredictor(Predictor):
    ...
    def vpsde_update_fn(self, x, t, context, classifier_fn=None):
        sde = self.sde
        timestep = (t * (sde.N - 1) / sde.T).long()
        beta = sde.discrete_betas.to(t.device)[timestep]
        score = self.score_fn(x, t, context, classifier_fn)
        # if classifier_fn is not None:
        #     # print(f'using classifier_fn, context {context}')
        #     score = score - classifier_fn(x, t, context)
    ...
    def update_fn(self, x, t, context, classifier_fn=None):
        if isinstance(self.sde, sde_lib.VPSDE):
            return self.vpsde_update_fn(x, t, context, classifier_fn=classifier_fn)
        else:
...
def shared_predictor_update_fn(x, t, sde, model, predictor, probability_flow, continuous, context=None, classifier_fn=None):
    ...
    return predictor_obj.update_fn(x, t, context, classifier_fn=classifier_fn)
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
                                     ...
                                     denoise=True, eps=1e-3, device='cuda', grid_mask=None, return_traj=False, classifier_fn=None):
    ...
    def pc_sampler(model, 
            ...
            freeze_iters=None, context=None):
        ...
        with torch.no_grad():
            ...
            def compute_xzero(sde, model, x, t, grid_mask_input):
                ...
                score_pred = model(x, t * torch.ones(shape[0], device=x.device), context)
            ...
            # np.save('x0.npy', x.cpu().numpy())
            # np.save('x1.npy', x.cpu().numpy())
            # random_0 = np.load('x0.npy')
            # random_1 = np.load('x1.npy')
            # x = torch.from_numpy(random_0 + random_1).to(device) / 2.
            
            # labels = torch.randint(0, sde.N, (shape[0],), device=device)
            # labels = torch.ones(shape[0], device=device, dtype=torch.long) * (sde.N - 1)
            # print(labels)
            # sqrt_alphas_cumprod = sde.sqrt_alphas_cumprod.to(device)
            # sqrt_1m_alphas_cumprod = sde.sqrt_1m_alphas_cumprod.to(device)
            # noise = torch.randn(shape).to(device)
            # batch = torch.load('/home/jialuo/CG_project/MeshDiffusion/nvdiffrec/car_1/grid/grid_00000.pt').to(device).unsqueeze(0).repeat(shape[0], 1, 1, 1, 1)
            # x = sqrt_alphas_cumprod[labels, None, None, None, None] * batch + \
