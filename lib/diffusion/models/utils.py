...
def get_model_fn(model, train=False):
  ...
  def model_fn(x, labels, context=None, use_text_context=False):
    ...
    if not train:
      model.eval()
      return model(x, labels, context, use_text_context)
    else:
      model.train()
      return model(x, labels, context, use_text_context)
...
def get_score_fn(sde, model, train=False, continuous=False, std_scale=True):
  ...
  if isinstance(sde, sde_lib.VPSDE):
    if not std_scale:
      def score_fn(x, t, context):
        labels = t * (sde.N - 1)
        score = model_fn(x, labels, context)
        return score
    else:
      def score_fn(x, t, context, classifier_fn=None):
        # For VP-trained models, t=0 corresponds to the lowest noise level
        labels = t * (sde.N - 1)
        score = model_fn(x, labels, context)
        if classifier_fn is not None:
          score = score - classifier_fn(x, labels, context)
